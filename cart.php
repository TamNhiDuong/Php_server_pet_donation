...
try{
	...
	if($decoded->expire < time()){
		...
		foreach ($arrayDetail as $value) {
			...
			//add donation amount to each pet
			$donationamount = $value['quantity']*$product['price'];
			$query = "UPDATE product SET totalDonation = totalDonation + $donationamount WHERE id=$id_sp";
			$insertdonation = $mysqli->query($query);
		}
